...
from app.logic.dining_room.CModelMinexpense import CModelMinexpense
from app.logic.dining_room.CDataMinexpense import CDataMinexpense
...
class CPopMinexpenseSetting ( wx.Dialog ):
    ...
    def __init__( self, parent ):
        ...
        self.model = CModelMinexpense(CDataMinexpenseInfo.GetData())
        ...
        self.m_dataViewListName = self.m_dataViewList.AppendTextColumn( u"消费名称", 2, width=-1, mode=wx.dataview.DATAVIEW_CELL_EDITABLE ) 
        self.m_dataViewListPrice = self.m_dataViewList.AppendTextColumn( u"消费金额", 3, width=-1, mode=wx.dataview.DATAVIEW_CELL_EDITABLE ) 
    ...
    def OnBtnNew( self, event ):
        event.Skip()
        CDataMinexpenseInfo.AddItem(CDataMinexpense(0, 0, "", 0))
        
        data = CDataMinexpense(CDataMinexpenseInfo.GetDataLen() + 1, CDataMinexpenseInfo.GetId(), "", 0)
    ...
    def OnBtnDelete( self, event ):
        ...
        CDataMinexpenseInfo.DeleteItem(data)
    
    def OnBtnRefresh( self, event ):
        event.Skip()
        
        result = CDataMinexpenseInfo.GetData()
    ...
    def OnBtnSave( self, event ):
        ...
        item = self.m_dataViewList.GetCurrentItem()
        data = self.model.ItemToObject(item)
        CDataMinexpenseInfo.UpdateItem(data)
...
if __name__ == '__main__':
    app = wx.PySimpleApp()
    dlg = CPopMinexpenseSetting(None)
    dlg.Show()
    #dlg.Center()
    app.MainLoop()
