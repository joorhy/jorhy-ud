...
from app.logic.employee.CModelDepartment import CModelDepartment
from app.logic.employee.CDataDepartment import CDataDepartment
...
class CPopDepartment ( wx.Dialog ):
    ...
    def __init__( self, parent ):
        ...
        self.model = CModelDepartment(CDataDepartmentInfo.GetData())
        ...
        self.m_dataViewListName = self.m_dataViewList.AppendTextColumn( u"行政部门", 2, width=-1, mode=wx.dataview.DATAVIEW_CELL_EDITABLE ) 
    ...
    def OnBtnNew( self, event ):
        event.Skip()
        CDataDepartmentInfo.AddItem(CDataDepartment(0, 0, ""))
        
        data = CDataDepartment(CDataDepartmentInfo.GetDataLen() + 1, CDataDepartmentInfo.GetId(), "")
    ...
    def OnBtnDelete( self, event ):
        ...
        CDataDepartmentInfo.DeleteItem(data)
    
    def OnBtnRefresh( self, event ):
        event.Skip()
        
        result = CDataDepartmentInfo.GetData()
    ...
    def OnBtnSave( self, event ):
        ...
        item = self.m_dataViewList.GetCurrentItem()
        data = self.model.ItemToObject(item)
        CDataDepartmentInfo.UpdateItem(data)
