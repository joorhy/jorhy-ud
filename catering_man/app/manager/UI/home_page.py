...
class WgtHomePage (wx.Panel):
    ...
    def Initailize(self):
        ...
        select_item = CtrlHomePage.GetSelectedItem()
        if select_item == 0:
    ...
    def OnDiningRoonSetting(self, event):
        event.Skip()
        CtrlHomePage.SetSelectedItem(0)
        self.ShowDiningRoonSetting()

    def OnDishesPublishing(self, event):
        event.Skip()
        CtrlHomePage.SetSelectedItem(1)
        self.ShowDishesPublishing()

    def OnEmployeeManager(self, event):
        event.Skip()
        CtrlHomePage.SetSelectedItem(2)
        self.ShowStaffManager()

    def OnPrinterSetting(self, event):
        event.Skip()
        CtrlHomePage.SetSelectedItem(3)
        self.ShowPrinter()

    def OnReportForms(self, event):
        event.Skip()
        CtrlHomePage.SetSelectedItem(4)
        self.ShowReportForms()

    def OnSystemSetting(self, event):
        event.Skip()
        CtrlHomePage.SetSelectedItem(5)
        self.ShowSystemSetting()
    ...
    def OnFunc_1(self, event):
        ...
        select_item = CtrlHomePage.GetSelectedItem()
        if select_item != 5:
            AppManager.SwitchToApplication(li_funcwidget_1[select_item])
        else:
            self.popCompany = PopCompany(self)
    ...
    def OnFunc_2(self, event):
        ...
        select_item = CtrlHomePage.GetSelectedItem()
        if select_item != 5:
            AppManager.SwitchToApplication(li_funcwidget_2[select_item])
        else:
            self.popRigster = PopRegister(self)
    ...
    def ShowDiningRoonSetting(self):
        ...
        AppManager.SwitchToApplication(li_funcwidget_1[0])
        self.SetFramTile()
    ...
    def ShowDishesPublishing(self):
        ...
        AppManager.SwitchToApplication(li_funcwidget_1[1])
        self.SetFramTile()
        self.ShowFuncBtns()
